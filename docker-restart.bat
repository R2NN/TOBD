@echo off
chcp 65001 >nul
REM ============================================================================
REM Скрипт перезапуска Docker контейнера для Windows
REM ============================================================================
color 0E
title Docker Restart - FastAPI Log Analyzer

echo.
echo ═══════════════════════════════════════════
echo  FastAPI Log Analyzer - Docker Restart
echo ═══════════════════════════════════════════
echo.

REM Проверка установки Docker
echo [1/3] Проверка Docker...
docker --version >nul 2>&1
if %errorlevel% neq 0 (
    color 0C
    echo [ERROR] Docker не установлен!
    pause
    exit /b 1
)

echo [OK] Docker найден
echo.

echo [2/3] Остановка всех контейнеров...
docker-compose down

if %errorlevel% neq 0 (
    color 0C
    echo [ERROR] Ошибка при остановке!
    echo.
    pause
    exit /b 1
)

echo [OK] Контейнеры остановлены
echo.

echo [3/3] Запуск всех контейнеров...
echo (может занять несколько минут)
echo.
docker-compose up -d --build

if %errorlevel% equ 0 (
    color 0A
    echo.
    echo ═══════════════════════════════════════════
    echo  [SUCCESS] Все контейнеры перезапущены!
    echo ═══════════════════════════════════════════
    echo.
    echo  🌐 Приложение доступно на:
    echo     http://localhost:8001
    echo     http://localhost:8001/docs
    echo.
    echo  📊 Prometheus:
    echo     http://localhost:9090
    echo.
    echo  📈 Grafana:
    echo     http://localhost:3000 (admin/admin)
    echo.
    echo  💡 Полезные команды:
    echo     docker-compose logs -f       - Просмотр логов
    echo     docker-compose ps            - Статус контейнеров
    echo     📊_ПРОВЕРИТЬ_СТАТУС.bat     - Диагностика
    echo.
    
    REM Ожидание запуска
    echo ⏳ Ожидание запуска (5 секунд)...
    timeout /t 5 /nobreak >nul
    
    REM Открываем браузер
    start http://localhost:8001
    
) else (
    color 0C
    echo.
    echo [ERROR] Ошибка при запуске контейнеров!
    echo.
    echo 💡 Попробуйте:
    echo    1. Убедитесь, что Docker Desktop запущен
    echo    2. Запустите: 🛑_ОСТАНОВИТЬ_ВСЁ.bat
    echo    3. Затем:     🚀_ЗАПУСТИТЬ_ВСЁ.bat
    echo.
)

pause

